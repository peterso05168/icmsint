...
import hk.judiciary.icmsint.model.sysinf.converter.gdsnid2j.SODApplicationV20CT_SysInfCaseDTO_Converter;
...
public class SODApplicationHandler extends BaseGDSNIMsgHandler {
	
	public SODApplicationHandler(JudiciaryUser judiciaryUser, String partyCd) {
		super(judiciaryUser, partyCd, SysInfConstant.SYSINF_MSG_CD_GDSNI_D2J_SOD_APPLICATION, SysInfConstant.SYSINF_MSG_TYPE_CD_GDSI, SysInfConstant.SYSINF_MSG_MSGNAT_IN);
	}
	...
	public CommonWsDTO handleMsg(GDSNIMsgD2J msg) throws SysInfHandlerException {
		int seqNo = 0;
        int failCnt = 0;
        for (SODApplicationV20CT SODApplication : msg.getSODApplication()) {
			try {
				SODApplicationV20CT_SysInfCaseDTO_Converter converter = new SODApplicationV20CT_SysInfCaseDTO_Converter();
	        	SysInfCaseDTO sysInfCaseDto = converter.convert(SODApplication);
				String fileName = genFileName(SODApplication);
				...
				try {
					...
					insertSysInfErr(insertSysInfRec(seqNo++, genFileName(SODApplication), SysInfStatus.FAIL.code()), errMsg);
	...
	private String genFileName(SODApplicationV20CT SODApplication) {
    	SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMddHHmmss");
    	Timestamp timestamp = new Timestamp(System.currentTimeMillis());
    	return getPartyCd() + "-" + "SODApplication" + "-" + sdf.format(timestamp);
	}
