...
public class WitnessSummonsApplicationHandler extends BaseGDSNIMsgHandler {
	
	public WitnessSummonsApplicationHandler(
			JudiciaryUser judiciaryUser, String partyCd) {
		super(judiciaryUser, partyCd, SysInfConstant.SYSINF_MSG_CD_GDSNI_D2J_WITNESS_SUMMONS_APPLICATION, SysInfConstant.SYSINF_MSG_TYPE_CD_GDSI, SysInfConstant.SYSINF_MSG_MSGNAT_IN);
	}
        ...
        for (WitnessSummonsApplicationV20CT witnessSummons : msg.getWitnessSummonsApplication()) {
			try {
				...
				try {
					...
					insertSysInfErr(insertSysInfRec(seqNo++, genFileName(witnessSummons), SysInfStatus.FAIL.code()), errMsg);
	...
	private String genFileName(WitnessSummonsApplicationV20CT witnessSummons) {
