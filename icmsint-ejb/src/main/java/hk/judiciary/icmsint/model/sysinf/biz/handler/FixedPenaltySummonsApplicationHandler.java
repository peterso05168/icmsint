...
import hk.judiciary.icmsint.model.sysinf.converter.gfpid2j.FixedPenaltySummonsV20CT_SysInfCaseDTO_Converter;
...
public class FixedPenaltySummonsApplicationHandler extends BaseGFPIMsgHandler {
	
	public FixedPenaltySummonsApplicationHandler(
			JudiciaryUser judiciaryUser, String partyCd) {
		super(judiciaryUser, partyCd, SysInfConstant.SYSINF_MSG_CD_GFPI_D2J_FIXED_PENALTY_SUMMONS_APPLICATION, SysInfConstant.SYSINF_MSG_TYPE_CD_GFPI, SysInfConstant.SYSINF_MSG_MSGNAT_IN);
	}
	...
	public CommonWsDTO handleMsg(GFPIMsgD2J msg) throws SysInfHandlerException {
		int seqNo = 0;
        int failCnt = 0;
        for (FixedPenaltySummonsV20CT fixedPenaltySummons : msg.getFixedPenaltySummons()) {
			try {
				FixedPenaltySummonsV20CT_SysInfCaseDTO_Converter converter = new FixedPenaltySummonsV20CT_SysInfCaseDTO_Converter();
	        	SysInfCaseDTO sysInfCaseDto = converter.convert(fixedPenaltySummons);
	        	String fileName = genFileName(fixedPenaltySummons);
				...
				try {
					...
					insertSysInfErr(insertSysInfRec(seqNo++, genFileName(fixedPenaltySummons), SysInfStatus.FAIL.code()), errMsg);
	...
	private String genFileName(FixedPenaltySummonsV20CT fixedPenaltySummons) {
